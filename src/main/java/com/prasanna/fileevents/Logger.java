...
public class Logger
{
	...
	public void printHeader()
	{
		...
		outputStream.format("|%1$-" + TIME_COLUMN_WIDTH + "s|%2$-" + ACTION_COLUMN_WIDTH + "s|%3$-"
		        + TYPE_COLUMN_WIDTH + "s|%4$-" + DETAILS_COLUMN_WIDTH + "s%5$s", "Occurence",
		        "Event", "Type", "Details", "|");
	...
	private void printMultiRowStatement(Event event, Action action, String text)
    {
	    ...
	    for (int i = 1; i <= numRows; i++)
	    {
	    	...
	    	logLine("|" + padRight("", TIME_COLUMN_WIDTH) + "|"
	    	        + padRight("", ACTION_COLUMN_WIDTH) + "|"
	    	        + padRight("", TYPE_COLUMN_WIDTH) + "|"
	    	        + padRight(rowText, DETAILS_COLUMN_WIDTH) + "|");
