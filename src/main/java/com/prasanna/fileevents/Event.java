...
public class Event
{
	...
	{
		boolean isDirectoryEvent = false;

		if (contentHash != null && contentHash.equals("-"))
		{
			isDirectoryEvent = true;
		}
	...
	public boolean isUnderParent(String parentPath)
	{
		...
		if (parentPath != null && path.length() > parentPath.length())
		{
			...
			while (tokenizer.hasMoreTokens())
			{
				...
				if (myPathPart == null || myPathPart.equals(parentPathPart) == false)
				{
					isUnderParent = false;
					break;
				}
	...
	{
		return "Event [eventType=" + eventType + ", timeStamp=" + timestamp + ", path=" + path
